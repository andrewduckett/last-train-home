...
header, schedule, places, scavenger tasks and rules, map URLs, and quick links. The provider
...
The YAML seed SHALL preserve the planned event's visible schedule wording, times, and entry order when its fields move to the generic itinerary schema. It SHALL preserve place and task content, link labels and destinations, map destinations, and their authored order. The seed SHALL use the new generic itinerary fields rather than retaining obsolete field names.
...
- **THEN** its place, task, link, and map content and destinations match the earlier event
