...
- [ ] 1.1 Move the existing data types into `src/lib/types.ts`; add `CrawlIdentity`, `CrawlDefinition`, `Crawl`, and the four `CrawlResult` variants from the design. Preserve every existing definition field and subtype; verify `npm run check` and the existing component tests pass.
- [ ] 1.2 Add `CrawlProvider` and its factory in `src/lib/data/provider.ts`, with an injected retrieval function. Work test-first: verify the real factory returns a promise containing `found` for an available record and sets the **crawl** id from the requested **logical id**.
- [ ] 1.3 Make the **provider** return `not-found` when retrieval returns `undefined`. Write the failing test first; verify the resolved result carries the requested id.
...
- [ ] 2.1 Require a string `title` in the **identity**. Work test-first through missing and non-string titles, including `null`; verify each resolves to `invalid` with the requested id.
- [ ] 2.2 Add regression tests for empty and whitespace titles. Verify each authored string survives in the `found` result unchanged; correct any failure without adding length checks or trimming.
- [ ] 2.3 Validate optional `date` and `color` by type alone. Work test-first through absent fields, strings, and present non-string values, including `null` and `undefined`; verify absent fields and strings are accepted while present non-strings resolve to `invalid`.
- [ ] 2.4 Add regression tests for uninterpreted date and color strings. Verify an unrecognized color and a non-date string survive unchanged; correct any failure without adding interpretation or palette resolution.
- [ ] 2.5 Carry the **definition** through unchanged, without validating its contents. Write failing tests first; verify every source field is preserved and valid identity still resolves to `found` when the scavenger list is `null`.
