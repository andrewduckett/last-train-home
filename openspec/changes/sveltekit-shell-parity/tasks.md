...
- [ ] 1.1 Add SvelteKit, Svelte 5, `@sveltejs/adapter-static`, `@sveltejs/vite-plugin-svelte`, TypeScript, `svelte-check`, Vitest, `@testing-library/svelte`, and `jsdom` to `package.json`; verify `npm install` succeeds.
- [ ] 1.2 Create `svelte.config.js` (adapter-static, `fallback: 'index.html'`), `vite.config.ts`, `tsconfig.json`, `src/app.html`, and `src/routes/+layout.ts` with `ssr = false` and `prerender = true`; verify `npm run check` runs clean.
- [ ] 1.3 Add `dev`, `build`, `preview`, `test`, and `check` scripts to `package.json`; verify `npm run dev` serves an empty shell and `npm run build` emits a `build/` directory.
