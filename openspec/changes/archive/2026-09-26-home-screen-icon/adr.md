...
- **No design choice binds later work.** Reversing any one of them edits one script, the manifest, or a few lines of `app.html`, and touches no crawl data or screen.
...
- **ADR 0007 (proposed) keeps every UI color in one palette source.** The generator reads the app icon colors from the palette. A test holds the one hand-written color, `theme-color` in `app.html`, equal to the palette's `board` color.
